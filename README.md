...
swap - see below). **Avg Entry MC** is the same data expressed as an implied
market cap: (Invested ÷ tokens acquired via swap) × the 1 trillion token
supply. The total row's figure is the weighted average across all
calculated wallets, not an average of the per-wallet averages.
